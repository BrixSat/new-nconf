...
# PHP 8.1+ makes mysqli throw exceptions by default; the installer checks return
# values / mysqli_error() instead, so keep the classic non-throwing behaviour.
    ...
    if ($db_check !== FALSE)
    {
        ...
        } else
        {
            ...
            if ($db_selected)
            {
                ...
                # PHP 8.1+ makes mysqli throw on error instead of returning FALSE.
                # A missing table is the expected case on a fresh install, so
                # swallow it and let $result be FALSE to trigger table creation.
                try
                {
...
} elseif ($step == 3)
{
    ...
    echo NConf_HTML::limit_space(
        NConf_HTML::show_highlight('Info', 'For more authentication setups, please reffer to the online documentation!'
            . '<br>Just finish your installation and then have a look here --> <a href="INSTALL/Authentication-options.pdf" target="_blank">Authentication @ nconf.org</a>')
    );
    ?>
    <script type="text/javascript">
        <!--
